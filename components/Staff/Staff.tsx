  ...
  return (
    ...
    >
      <div className="mx-auto px-6 md:px-10" style={{ maxWidth: '900px' }}>
        ...
        <div className="relative mt-14" style={{ maxWidth: '640px', margin: '3.5rem auto 0' }}>
          {/* Floating background coordinates */}
          <div className="absolute -top-12 -left-12 opacity-10 select-none text-[10px] font-mono tracking-widest hidden lg:block">
            LAT: 30.5401° N<br />LON: 77.2917° E<br />ALT: 1,120M
          </div>
          
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-8 gap-y-12 relative z-10">
            {staffMembers.map((member, i) => {
              const offsetClass = i % 3 === 1 ? 'sm:translate-y-6' : i % 3 === 2 ? 'sm:-translate-y-3' : '';
              return (
                <motion.div
                  key={member.id}
                  ...
                  className={`flex flex-col items-center text-center sm:transform ${offsetClass}`}
                >
                  {/* Avatar — circle */}
                  <div
